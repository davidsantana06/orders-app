...
export class OrderItemService {
  ...
  getMany(): Observable<OrderItem[]> {
    return this.http.get<OrderItem[]>(this.apiUrl).pipe(catchError(handleHttpError));
  }

  getById(id: number): Observable<OrderItem> {
    return this.http.get<OrderItem>(`${this.apiUrl}/${id}`).pipe(catchError(handleHttpError));
  }
  ...
  create(dto: CreateOrderItemDto, orderId: number): Observable<OrderItem> {
    ...
    return this.http
      .post<OrderItem>(this.apiUrl, dto, { params })
      .pipe(catchError(handleHttpError));
  }

  update(id: number, dto: UpdateOrderItemDto): Observable<OrderItem> {
    return this.http.put<OrderItem>(`${this.apiUrl}/${id}`, dto).pipe(catchError(handleHttpError));
  }
